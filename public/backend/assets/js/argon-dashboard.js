...
if(sessionStorage.getItem('darkmode')){
  ...
  $('#DarkmodeOFFON').html(`<input class="form-check-input mt-1 ms-auto" type="checkbox" checked  value="on" id="dark-version" onclick="darkMode(this)">`);
  ...
  $('#DarkmodeOFFON').html(`<input class="form-check-input mt-1 ms-auto" type="checkbox"  value="off" id="dark-version" onclick="darkMode(this)">`);
  const body = document.getElementsByTagName('body')[0];
  const hr = document.querySelectorAll('div:not(.sidenav) > hr');
  const sidebar = document.querySelector('.sidenav');
  const sidebarWhite = document.querySelectorAll('.sidenav.bg-white');
...
function darkMode(el) {
 if(el.value==='on'){
  ...
  sessionStorage.setItem("darkmode", el.getAttribute("checked"));
  location.reload()
 }
